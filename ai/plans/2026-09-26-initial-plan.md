...
| Search settings | Scan length, max results and Yomitan-style text replacements (regex groups, original text searched too), editable in the app. Romaji is converted to hiragana like Yomitan's `alphabeticToHiragana` (the only Yomitan Japanese preprocessor hoshidicts lacks); matched lengths map back to the original characters |
